...
export function RadioEqualizer({ mediaRef, isRadio, volume, onWebAudioRoutingActive }: RadioEqualizerProps) {
  const uid = useId();
  ...
  const ensureGraph = useCallback(async () => {
    ...
    if (!el || !isRadio || graphRef.current || buildingRef.current) return;
  ...
  }, [isRadio, mediaRef, onWebAudioRoutingActive]);

  useEffect(() => {
    if (!open || !isRadio) return;
    void ensureGraph();
  }, [open, isRadio, ensureGraph]);
